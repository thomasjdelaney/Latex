...
To find order in the correlated behaviour of a large multi-region neuronal ensemble, we applied a novel method from network science to detect structure and communities in correlated behaviour. We investigated the similarities between these communities and their brain anatomy. Our results indicate local correlated networks function at shorter timescales ($<50$ms), while multi-region correlated networks function over longer timescales ($>100$ms). This result agrees with previous findings from EEG data, but has not been shown before using spiking data.
...
% $\mathbf{I(X;Y)}$ & Mutual information between $X$ and $Y$ \\
